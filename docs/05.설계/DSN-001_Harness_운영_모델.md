...
이 문서는 JKADH Harness가 작업 입력을 받아 에이전트 배치, 상태 판단, 승인, 산출물 연결을 운영하는 모델에 적용한다.

구체적인 저장소 API, 데이터 엔티티, 구현 방식은 별도 설계 문서에서 다룬다.
...
| 상태 | 한글명 | 의미 | 다음 상태 |
|---|---|---|---|
| Candidate | 후보 | Issue 또는 Backlog에서 작업 후보를 읽은 상태 | Intake(접수), Backlog(백로그) |
| Intake | 접수 | 목적, 범위, 완료 조건을 확인하는 상태 | Work Order Build(작업지시서 작성), Need Clarification(보완 필요), Backlog(백로그) |
| Work Order Build | 작업지시서 작성 | 사용자 요청을 실행 가능한 Work Order 초안으로 작성하는 상태 | Work Order Review(작업지시서 검토), Need Clarification(보완 필요), Backlog(백로그) |
| Work Order Review | 작업지시서 검토 | Work Order 초안을 피드백받고 실행 가능한지 확인하는 상태 | Classify(분류), Work Order Build(작업지시서 작성), Need Approval(승인 필요), Backlog(백로그) |
| Classify | 분류 | 작업 성격, 난이도, 위험도, 권한 영향을 분석하는 상태 | Decompose(분해), Gate Check(게이트 확인), Need Approval(승인 필요), Backlog(백로그) |
| Decompose | 분해 | 작업 분량을 분석하고 필요한 경우 세부 태스크로 분리하는 상태 | Gate Check(게이트 확인), Backlog(백로그) |
| Gate Check | 게이트 확인 | 권한 게이트와 Token Budget을 확인하는 상태 | Assign(배정), Need Approval(승인 필요), Stop(중단) |
| Assign | 배정 | 필요한 에이전트 역할과 AI 개발자 후보를 정하는 상태 | Execute(실행) |
| Execute | 실행 | AI 개발자에게 작업을 지시하고 산출물을 받는 상태 | Collect(수집) |
| Collect | 수집 | 산출물을 수집하고 누락 여부를 확인하는 상태 | Verify(검증), Backlog(백로그) |
| Verify | 검증 | 완료 조건과 검증 근거를 확인하는 상태 | Accept Candidate(수용 후보), Review(리뷰), Need Approval(승인 필요), Backlog(백로그) |
| Accept Candidate | 수용 후보 | Low-risk 결과를 Harness가 수용 후보로 판단한 상태 | Report(보고), PR |
| Need Approval | 승인 필요 | 사람 리드의 승인이 필요한 상태 | Execute(실행), Report(보고), Stop(중단) |
| Review | 리뷰 | 리뷰 또는 QA 보강이 필요한 상태 | Execute(실행), Report(보고), Backlog(백로그) |
| Report | 보고 | 결과, 근거, 후속 작업을 보고로 정리하는 상태 | Closed(종료) |
| Backlog | 백로그 | 현재 범위 밖 항목을 분리한 상태 | Closed(종료) |
| Stop | 중단 | 권한, Token, 범위, 검증 문제로 중단한 상태 | Report(보고), Closed(종료) |
...
| Secret, 배포, main 영향 작업이 승인되지 않음 | Stop(중단) 또는 Need Approval(승인 필요) |
...
승인 조건에 해당하지 않는 경우에도 Harness는 승인 항목을 확인했다는 기록을 남긴다. 이를 Approval Pass(승인 통과)로 부르며, 실행 전에 사람 승인 필수 조건이 없었다는 근거를 남기는 용도로 사용한다.
...
| 승인 확인 | 필수 | Approval Pass(승인 통과) 또는 Need Approval(승인 필요) 판단이다. |
