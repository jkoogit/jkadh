...
> Snapshot: 2026-07-16 HCP DB 테이블 확장 반영 시점의 DSN-008 history snapshot.
> 원본 최신본은 `../DSN-008_DB_테이블_설계서.md`에서 계속 관리한다.

## Snapshot 메모

| 항목 | 값 |
|---|---|
| Snapshot ID | DSN-008-HIST-001 |
| Snapshot 일자 | 2026-07-16 |
| 기준 Issue | [#84](https://github.com/jkoogit/jkadh/issues/84) |
| 기준 브랜치 | `main` |
| 작업 브랜치 | `task_codex/084-dsn-008-history-snapshot` |
| 기준 커밋 | `3179be4e7e1e1cda66fb2c5f7aac14e10c5df485` |
| 반영 상태 | HCP session/task/PR/backlog/issue/branch 테이블 migration, baseline, test 승급 완료 |
| 보류 범위 | HCP DB write-store 전환, runtime JSON 제거, dev/stg/prd DB reset 실행, Issue #84 종료 |

### Snapshot 요약

- `hcp.harness_session`, `hcp.harness_session_event`는 세션 상태와 세션 이벤트 저장소 기준으로 승급 완료됐다.
- `hcp.harness_task`, `hcp.harness_task_event`는 태스크 상태와 태스크 이벤트 저장소 기준으로 승급 완료됐다.
- `hcp.harness_pull_request`, `hcp.harness_backlog_item`은 PR 추적과 runtime backlog 추적 기준으로 승급 완료됐다.
- `hcp.harness_issue`, `hcp.harness_branch`는 GitHub Issue snapshot과 작업/승급 브랜치 추적 후보에서 실제 migration/baseline/test 반영 상태로 승급 완료됐다.
- `.hcp` runtime JSON과 DB 테이블 간 필드 매핑은 DSN-009로 별도 초안화했다.
- 본 snapshot은 DSN-008 본문 자체를 변경하지 않고, 2026-07-16 현재 설계 판단과 보류 범위를 고정 기록한다.
...
| 2026-07-16 | [#84](https://github.com/jkoogit/jkadh/issues/84) | Codex | GPT-5 | CTO | jk / Codex | Snapshot | HCP session/task/PR/backlog/issue/branch 테이블 확장과 DSN-009 JSON DB 매핑 설계 작성 이후 첫 history snapshot 생성 |
