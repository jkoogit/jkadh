...
Issue 종료는 완료 조건 충족, 후속 Backlog 분리, 남은 범위 없음이 확인된 뒤 `#태스크정리` 또는 `#세션정리`에서 별도 판단한다.
...
| Issue 종료 | 완료 조건 충족, 관련 PR 반영 완료, 남은 Backlog 분리, 종료 요청 또는 명시 승인 존재 | `#태스크승급` 중 자동 종료 시도, 완료 조건 일부 미충족, 후속 작업 미분류, 자동 종료 여부 불명확 |
