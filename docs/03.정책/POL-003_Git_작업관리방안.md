...
`#세션정리`에서 현재 브랜치의 `OPEN` PR이 감지되면 자동 재사용하지 않고 보류한다. 기존 PR 재사용은 head branch가 현재 세션정리 브랜치이고, PR title/body와 변경 파일이 세션정리 산출물 범위이며, 사용자가 `#세션정리.PR재사용`으로 명시 승인한 경우에만 허용한다. 이미 `MERGED` 상태인 PR은 신규 세션정리 커밋 대상으로 재사용하지 않는다.
...
```text
PR merge to dev (`merge_pr_to_dev`)
↓
...
게이트를 통과하지 못하면 상태 변경을 실행하지 않는다. 이미 일부 단계가 진행된 경우에는 [STA-002 AI 시작가이드](../00.시작/STA-002_AI_시작가이드.md)의 HCP 실행 게이트와 [POL-005 사람 승인 경계 정책](./POL-005_사람_승인_경계_정책.md)의 전환 기준에 따라 사후 보정, `Need Approval`, `Stop`, `Backlog` 중 하나로 정리한다.
