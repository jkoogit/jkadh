...
export function buildHarnessScopeGraphMarkdown(): string {
  return [
    ...
    "```mermaid",
    "flowchart TD",
    ...nodes.map((item) => `  ${item.id}["${item.displayOrder} ${item.label}"]`),
    ...nodes.flatMap((item) => item.parentIds.map((parentId) => `  ${parentId} --> ${item.id}`)),
    "```"
  ].join("\n") + "\n";
}
...
function cloneNode(item: HarnessScopeNode): HarnessScopeNode {
  return { ...item, parentIds: [...item.parentIds] };
}
